...
// Lookup table otomatis
const _lut = Array.from({length: 256}, (_, i) => (i < 16 ? '0' : '') + i.toString(16));
...
function inverseLerp(x, y, value) { return x !== y ? (value - x) / (y - x) : 0; }
...
function setQuaternionFromProperEuler(q, a, b, c, order) {
    ...
    switch(order){
        ...
        default: warn('GlazeMathUtils: unknown order: ' + order);
    }
}
...
function denormalize(value, array){
    switch(array.constructor){
        ...
        case Int32Array: return Math.max(value/2147483647, -1.0);
        case Int16Array: return Math.max(value/32767, -1.0);
        case Int8Array: return Math.max(value/127, -1.0);
        default: throw new Error('Invalid component type.');
    }
}

// Object MathUtils sebagai namespace
const MathUtils = {
    DEG2RAD, RAD2DEG, generateUUID, clamp, euclideanModulo, mapLinear, inverseLerp, lerp,
    damp, pingpong, smoothstep, smootherstep, randInt, randFloat, randFloatSpread,
    seededRandom, degToRad, radToDeg, isPowerOfTwo, ceilPowerOfTwo, floorPowerOfTwo,
    setQuaternionFromProperEuler, normalize, denormalize
};

// Export semua secara individual + MathUtils object
export {
    DEG2RAD, RAD2DEG, generateUUID, clamp, euclideanModulo, mapLinear, inverseLerp, lerp,
    damp, pingpong, smoothstep, smootherstep, randInt, randFloat, randFloatSpread,
    seededRandom, degToRad, radToDeg, isPowerOfTwo, ceilPowerOfTwo, floorPowerOfTwo,
    setQuaternionFromProperEuler, normalize, denormalize, MathUtils
};
